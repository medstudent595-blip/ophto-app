export const hsvCourseMarkdown = `
# 🔬 Herpès Oculaire : Physiopathologie, Clinique et Thérapeutique

> [!IMPORTANT]
> **Objectif DEMS 2025** : Ce cours magistral fusionne de manière exhaustive les référentiels de 2014 et leurs dernières mises à jour majeures de 2025. Prenez des notes, c'est fondamental.

---

## I. Épidémiologie et Physiopathologie : La dualité Virus/Immunité

L'herpès oculaire est un véritable fléau de santé publique. L'incidence des kératites varie entre 12 et 31,5 cas annuels pour 100 000 habitants en France, ce qui représente environ 18 000 épisodes par an.

*   **Séroprévalence** : Plus de 90 % de la population de plus de 60 ans est infectée de façon latente par le Virus Herpès Simplex de type 1 (HSV-1).
*   **Risque au cours d'une vie** : 1 % de risque de développer un herpès oculaire.

> [!WARNING]
> **Le nœud du problème : la récidive.** Le risque après un premier épisode est de 10 % à 1 an, 20 à 30 % à 2 ans, 40 % à 5 ans, et culmine à 60 à 70 % à 20 ans. La fréquence des récidives s'accélère d'ailleurs avec le nombre de poussées.

### Pathogénie
La contamination par HSV-1 (salive, gouttelettes) a lieu dans l'enfance. Le virus entre en latence dans le **ganglion de Gasser** (trigéminé).
Sous l'effet de facteurs déclenchants (ultraviolets, chirurgie, stress, fièvre, corticoïdes, analogues de prostaglandines), le virus se réactive dans le territoire V1 ophtalmique (0,15 % de la population).

La maladie clinique résulte d'une double composante qu'il faut comprendre pour traiter :
1.  🔴 **La toxicité virale directe (réplication)** : responsable des ulcérations épithéliales et de la nécrose tissulaire.
2.  🔵 **La réaction immunitaire (lymphocytaire et complexes immuns)** : indispensable pour éliminer le virus, mais responsable de l'inflammation délétère (infiltrats stromaux, fibrose, œdème).

---

## II. Sémiologie et Formes Cliniques Détaillées

> [!NOTE]
> **Signe clinique transversal majeur** : L'hypoesthésie ou anesthésie cornéenne unilatérale. La bilatéralité est rare (moins de 5 %) et se voit surtout chez l'enfant ou l'atopique.

### A. Atteintes palpébrales, conjonctivales et sclérales
*   **Blépharite** : Vésicules sur fond érythémateux évoluant vers des croûtes, classiquement sans cicatrice. Pensez au syndrome de Kaposi-Juliusberg chez le sujet atopique.
*   **Conjonctivite** : Sous-estimée (pourrait représenter 20 % des conjonctivites aiguës non bactériennes), elle associe souvent une adénopathie prétragienne. Cherchez systématiquement une ulcération conjonctivale dendritique.
*   **Sclérite** : Rare, souvent antérieure, nodulaire ou diffuse, liée à une réaction granulomateuse répondant lentement aux antiviraux.

### B. Kératites Épithéliales (Réplication pure)
Elles témoignent de l'effet cytopathogène du virus.
*   **Kératite Dendritique** (50 % des cas) : Ulcération linéaire fractale, arborisée, avec des renflements en bulbe à ses extrémités (différence clé avec le zona). Examen à la fluorescéine : le colorant imprègne le fond de l'ulcère puis diffuse en "brouillard verdâtre" dans le stroma adjacent en raison de l'altération de la membrane basale.
*   **Kératite Géographique** (10 % des cas) : Vaste placard ulcéreux, aux bords géographiques. C'est l'évolution redoutée d'une dendrite mal traitée ou, pire, aggravée par une corticothérapie intempestive.
*   **Marginale** : Ulcération périlimbique associée à un infiltrat stromal. Très douloureuse, elle résiste souvent au traitement antiviral seul (forme mixte).
*   **En archipel** : Forme mixte de disposition radiaire laissant des îlots de cornée saine.

### C. Kératites Stromales
*   🔴 **Nécrosante (Urgence absolue)** : Réplication virale massive au sein du stroma. Infiltrat dense blanc-jaunâtre, risque de descemétocèle et de perforation rapide.
*   🟠 **Non nécrosante** : Origine immunitaire. Infiltrat stromal diffus ou focal sans ulcération. Le risque est la baisse de vision à long terme par fibrose cicatricielle (taie) et néovascularisation.

### D. Endothélites (Mixte)
Association d'un œdème stromal sans infiltrat et de précipités rétro-descemétiques (PRD).
*   **Disciforme** : Œdème circulaire central, avec trabéculite et hypertonie oculaire satellite.
*   **Diffuse** : Œdème sur l'ensemble de la cornée.
*   **Linéaire** : PRD disposés en ligne progressant de la périphérie vers le centre, mimant le rejet de greffe de cornée. Forme très sévère avec perte endothéliale rapide.

### E. Uvéites Antérieures
Souvent récidivantes, unilatérales, sévères, associées ou non à la kératite.
*   **Signes pathognomoniques** : L'atrophie irienne en secteur (en "rayon de roue") transilluminable.
*   **Hypertonie oculaire majeure** (trabéculite virale) cédant rapidement sous antiviraux.
*   **PRD granulomateux** (en peau de léopard) avec parfois un signe d'Amsler positif à la ponction. L'évolution vers le glaucome secondaire est fréquente (28 à 45 % des cas).

---

## III. Outils Diagnostiques (Mise à jour 2025)

Le diagnostic est principalement clinique. La biologie intervient en cas de doute, de résistance, ou d'uvéite.

*   **La PCR en temps réel (quantitative)** : A supplanté les anciennes cultures cellulaires. Réalisée sur les larmes (kératite épithéliale) ou l'humeur aqueuse (uvéite), elle permet d'évaluer la charge virale.
*   **La PCR multiplex** : Permet de rechercher simultanément le HSV-1, le HSV-2, le VZV, l'EBV et le CMV sur un même prélèvement.
*   **Coefficient de Witmer-Desmont** : Rapport de production locale d'anticorps dans l'humeur aqueuse, très utile au décours de l'épisode aigu.
*   **Le séquençage génétique** : Recommandé désormais face à une récidive sévère sous traitement bien conduit pour identifier les souches mutantes résistantes aux antiviraux classiques.

---

## IV. Stratégie Thérapeutique Codifiée

### A. L'Arsenal thérapeutique médical et chirurgical

**Antiviraux Locaux :**
*   **Trifluridine (Virophta 1%)** : Très efficace sur toutes les souches mais très toxique pour l'épithélium car non sélectif de la cellule infectée.
*   **Aciclovir (Zovirax pommade 3%) & Ganciclovir (Virgan gel 0,15%)** : Prodrogues activées par la thymidine kinase virale. Extrêmement sélectives et moins toxiques, bien que des résistances soient possibles (souches à TK altérée).

**Antiviraux Généraux (Per Os / IV) :**
*   **Aciclovir** : 5 prises par jour. Adapter à la fonction rénale.
*   **Valaciclovir (Zelitrex)** : Prodrogue de l'aciclovir avec excellente biodisponibilité (2 prises par jour en curatif).
*   **Famciclovir** : Prodrogue du penciclovir, nouvelle alternative hors AMM en France mais très prometteuse.

**Traitements adjuvants & Innovations (2025) :**
*   **Lavage au sérum physiologique, larmes artificielles** (lessivage viral et traitement de la sécheresse induite), **Cycloplégiques** (douleur).
*   **Ciclosporine A collyre (2%)** : Immunomodulateur majeur pour les kératites stromales corticodépendantes.
*   **Anti-VEGF topique / ARN antisens** : Pour traiter la néovascularisation stromale rebelle.
*   **Sérum autologue ou PRP (Plasma riche en plaquettes)** : Pour les kératites neurotrophiques séquellaires.
*   **Greffe de Membrane Amniotique (GMA)** : Sauvetage en phase aiguë des nécroses stromales sévères.

### B. Les Règles d'Or de la Corticothérapie

> [!CAUTION]
> **CONTRE-INDICATION ABSOLUE (DANGER DE FONTE CORNÉENNE) :**
> *   Kératites Épithéliales (dendrite, géographique)
> *   Kératites Stromales Nécrosantes

> [!TIP]
> **INDICATIONS (Dexaméthasone topique) :**
> *   Kératites stromales non nécrosantes, endothélites (sauf linéaires en première intention) et uvéites.
> 
> **CONDITION SINE QUA NON :** La réplication virale doit être contrôlée. Ne jamais débuter avant 48h d'antiviral efficace à dose curative. La décroissance des stéroïdes doit être extrêmement lente (paliers de plusieurs semaines) sous stricte couverture antivirale.

### C. Prophylaxie au long cours et gestion des résistances

Directement issue de l'étude HEDS (Herpetic Eye Disease Study).
*   **Indications** : Toute maladie récurrente grave (≥ 3 épisodes épithéliaux ou ≥ 2 stromaux sur 12 mois). En péri-opératoire (cataracte, PKR, greffe de cornée) pour contrer le stress neuronal et les corticoïdes.
*   **Posologie** : Valaciclovir 500 mg x1/j ou Aciclovir 400 mg x2/j pendant 12 mois minimum.
*   **Alerte Résistance (Mise à jour)** : Toute récidive épithéliale rapide sous prophylaxie bien conduite doit faire suspecter une résistance (mutation TK) nécessitant une documentation biologique.

---

## 🧠 Synthèse de la Prise en Charge

<div className="table-wrapper">
| Forme Clinique | Dominante Pathogénique | Traitement d'Attaque (1ère intention) | Place des Corticoïdes Topiques |
| :--- | :--- | :--- | :--- |
| **Épithéliale** | Réplication Virale | Valaciclovir PO (2x500mg/j) OU Ganciclovir local (5x/j) | 🛑 **JAMAIS** (Risque ulcère géographique) |
| **Stromale Nécrosante** | Réplication Massive | Aciclovir IV en urgence (hospitalisation) ± GMA | 🛑 **JAMAIS** en phase aiguë (Risque perforation) |
| **Stromale Non Nécrosante** | Immunologique | Valaciclovir PO (2x500mg/j à 3g/j) PUIS | ✅ **OUI**, après 48h-72h, décroissance très lente |
| **Endothélite Disciforme/Diffuse** | Immunitaire / Mixte | Valaciclovir PO (pleine dose) PUIS | ✅ **OUI**, sous stricte couverture antivirale |
| **Endothélite Linéaire** | Réplication / Mixte | Aciclovir IV en urgence | ✅ **OUI**, mais retardé (preuve efficacité antivirale) |
| **Uvéite Antérieure** | Trabéculite Virale / Mixte | Valaciclovir PO (pleine dose) | ✅ **OUI**, rapidement, sous couverture antivirale |
</div>
\`;

export const hsvFichesMarkdown = \`
# 📋 Fiche Technique : Herpès Oculaire

> [!IMPORTANT]
> **Objectif Pratique** : Cette fiche technique détaille les posologies exactes, les protocoles d'urgence et les règles strictes de corticothérapie pour la prise en charge de l'herpès oculaire.

---

## I. Bilan Diagnostique Pratique

### 1. L'examen à la lampe à fente (LAF)
*   **Test de sensibilité cornéenne** : À faire **AVANT** toute instillation de collyre anesthésique. L'hypoesthésie est le maître symptôme.
*   **Test à la Fluorescéine** : Recherche de dendrites (arbres morts avec renflements en bulbe aux extrémités).
    > [!TIP]
    > **Astuce clinique** : Le colorant stagne d'abord au fond de l'ulcère, puis diffuse en brouillard verdâtre dans le stroma (signe l'atteinte de la membrane basale, typique de l'herpès).
*   **Prise de Tonus (PIO)** : Une hypertonie (même relative de +4 mmHg par rapport à l'œil adelphe) est très évocatrice d'une trabéculite herpétique.

### 2. Les Prélèvements Biologiques (Mise à jour 2025)
Le diagnostic est clinique, mais la biologie est requise en cas de doute, de résistance ou de forme uvéitique/endothéliale grave.

*   **Forme de surface (Épithéliale)** : Grattage cornéen ou prélèvement de larmes pour PCR quantitative ou PCR multiplex. Un séquençage est demandé si l'on suspecte une résistance (mutation de la thymidine kinase virale). Évitez la fluorescéine avant le prélèvement car elle peut interférer avec la PCR.
*   **Forme intraoculaire (Uvéite/Endothélite)** : Ponction de chambre antérieure (PCA). Demander une PCR HSV dans l'humeur aqueuse (sensibilité 80%, spécificité proche de 100% en phase aiguë) et le calcul du Coefficient de Witmer-Desmont (utile à distance de l'épisode).

---

## II. Posologies et Ordonnances Types (Les Antiviraux)

> [!WARNING]
> Vous devez prescrire soit un traitement local, soit un traitement per os. Ils ont une efficacité équivalente pour les formes épithéliales, **il ne faut pas les associer** (risque de toxicité accrue).

### 1. Les Antiviraux Topiques (Locaux)
*   **Ganciclovir 0,15 % (Virgan® gel)** : 1 goutte 5 fois/jour jusqu'à cicatrisation, puis 3 fois/jour pendant 7 jours. Excellente tolérance.
*   **Aciclovir 3 % (Zovirax® pommade)** : 1 application (grain de riz) 5 fois/jour, à poursuivre 3 jours après cicatrisation. Gêne visuelle importante.
*   **Trifluridine 1 % (Virophta® collyre)** : 1 goutte toutes les 2 heures (max 9 gouttes/jour) jusqu'à cicatrisation, puis 5 fois/jour pendant 7 jours. Durée totale < 21 jours. Très épithéliotoxique, agit indépendamment de la thymidine kinase virale.

### 2. Les Antiviraux Per Os (Généraux)
Indispensables pour les formes stromales, endothéliales, les uvéites, ou les kératites épithéliales pédiatriques. **Toujours vérifier la clairance de la créatinine.**
*   **Valaciclovir (Zelitrex®)** : 1 comprimé de 500 mg, 2 fois/jour pendant 7 à 14 jours. Dans les endothélites diffuses ou formes sévères, la dose peut être montée hors AMM à 3 g/jour (1g x3/j).
*   **Aciclovir (Zovirax®)** : 2 comprimés de 200 mg (soit 400 mg), 5 fois/jour pendant 7 à 14 jours.
*   **Famciclovir (Oravir®)** : Hors AMM en France pour l'œil. 1 comprimé de 500 mg, 3 fois/jour pendant 10 jours.

### 3. Les Antiviraux Intraveineux (Urgence absolue)
*   **Aciclovir IV** : 5 à 10 mg/kg toutes les 8 heures.
*   **Indications** : Kératites stromales nécrosantes (hospitalisation), endothélites linéaires, ou uvéites avec atteinte endothéliale/rétinienne majeure.

---

## III. Règles de Maniement des Corticoïdes Locaux

🚨 C'est ici que l'erreur médicale vous coûtera votre examen (et la cornée de votre patient).

> [!CAUTION]
> **🚫 CONTRE-INDICATIONS ABSOLUES :**
> *   Toute lésion épithéliale active (dendrite, ulcère géographique).
> *   Kératite stromale nécrosante (risque de fonte purulente et perforation).

> [!NOTE]
> **✅ INDICATIONS STRICTES :**
> *   Kératites stromales non nécrosantes.
> *   Endothélites disciformes et diffuses.
> *   Uvéites antérieures herpétiques.

> [!WARNING]
> **⚠️ PROTOCOLE D'INTRODUCTION (La Règle d'Or) :**
> *   **Couverture antivirale préalable** : Ne JAMAIS débuter avant 48 heures d'un traitement antiviral systémique efficace (ex: Valaciclovir).
> *   **Dose d'attaque** : Dexaméthasone collyre, 6 à 8 gouttes par jour.
> *   **Décroissance** : Extrêmement lente (paliers de plusieurs semaines). La couverture antivirale doit être maintenue jusqu'au sevrage total des corticoïdes.

---

## IV. Traitements Adjuvants et Innovations

*   **Débridement épithélial** : À la LAF avec une éponge stérile. Réduit la charge virale et accélère la cicatrisation épithéliale. Synergique avec les antiviraux.
*   **Cycloplégiques** : Atropine 1 % (1 goutte 2 à 3 fois/j) pour lever le spasme ciliaire et calmer la douleur.
*   **Larmes artificielles sans conservateur** : Essentielles pour lessiver le virus et traiter la sécheresse induite par l'hypoesthésie.
*   **Cas des Corticodépendances** : Utilisation de Ciclosporine A en collyre à 2 % (hors AMM) pour prendre le relais des stéroïdes et éviter le rebond inflammatoire.
*   **Sauvetage tissulaire** : Greffe de membrane amniotique (GMA) pour les nécroses stromales sévères ou descemétocèles.

---

## V. Protocoles de Prophylaxie (Prévention des récidives)

*   **Indications** : 
    *   Maladie herpétique très récurrente (≥ 3 épisodes épithéliaux ou ≥ 2 épisodes stromaux par an).
    *   Prévention périopératoire : chirurgie de la cataracte, chirurgie réfractive (PKR, LASIK), greffe de cornée (PK, DMEK).
*   **Posologie (au long cours, 6 à 12 mois minimum)** :
    *   **Valaciclovir** : 1 comprimé de 500 mg / jour.
    *   **Aciclovir** : 2 comprimés de 200 mg (400 mg), 2 fois / jour (soit 800 mg/j au total).
*   **Suivi Biologique (Aciclovir/Valaciclovir per os)** : Créatininémie, bilan hépatique (ASAT/ALAT) et NFS-Plaquettes à M1 puis tous les 2 à 3 mois.

---

## 🧠 Récapitulatif : Ordonnances Types Selon La Forme

<div className="table-wrapper">
| Forme Clinique | Traitement d'Attaque | Corticoïdes Topiques ? |
| :--- | :--- | :--- |
| 🟢 **Kératite Épithéliale** | Valaciclovir PO (500mg x2/j) **OU** Ganciclovir local (5x/j) <br/> + Atropine + Larmes. (Durée: 7-14j) | 🛑 **Zéro Corticoïde** |
| 🔴 **Stromale Nécrosante** /<br/> **Endothélite Linéaire** | **Aciclovir Intraveineux** (Hospitalisation) <br/> + Larmes + Cycloplégique | 🛑 **Zéro Corticoïde** (en phase aiguë) |
| 🟠 **Stromale Non Nécrosante** /<br/> **Endothélite Disciforme** | Valaciclovir PO (500mg x2/j) | ✅ **OUI**, ajout de Dexaméthasone topique (6-8x/j) **après 48h**. Décroissance très lente. |
| 🟣 **Uvéite Antérieure** | Valaciclovir PO pleine dose <br/> + Hypotonisant (Éviter les prostaglandines) + Cycloplégique | ✅ **OUI**, Dexaméthasone topique rapidement |
| 🛡️ **Prophylaxie** | Valaciclovir PO (500mg x1/j) sur 1 an | - |
</div>
`;

export const hsvAlgorithmsMarkdown = `
# 🧠 Algorithme Décisionnel et de Prise en Charge (Mise à jour 2025)

> [!IMPORTANT]
> L'algorithme commence systématiquement par une question fondamentale à la lampe à fente (après test de sensibilité) : **Y a-t-il une atteinte de l'épithélium (prise de fluorescéine) ?**

---

### 🟩 BRANCHE 1 : ATTEINTE ÉPITHÉLIALE PRÉSENTE (Dendritique, Géographique)
La réplication virale est active et en surface.

*   **Décision 1** : 🛑 **CONTRE-INDICATION ABSOLUE AUX CORTICOÏDES**.
*   **Décision 2** : Choix du traitement antiviral d'attaque (Ne jamais associer local et per os).
    *   *Option A (Local)* : Ganciclovir gel 0,15 % (5x/jour).
    *   *Option B (Per Os)* : Valaciclovir 500 mg (1 comp 2x/jour).
*   **Suivi** : Contrôle à J5-J7.
    *   *Si guérison* : Poursuivre le traitement local à 3x/jour pendant 7 jours, ou arrêter le per os selon cicatrisation.
    *   *Si non-guérison/aggravation* : Suspecter une toxicité médicamenteuse, un problème neurotrophique ou, plus rarement, une résistance.

---

### 🟦 BRANCHE 2 : PAS D'ATTEINTE ÉPITHÉLIALE (Lésion Stromale ou Endothéliale)
L'épithélium est intact. Il faut évaluer la sévérité de l'atteinte profonde.

#### Sous-Branche 2A : Formes Sévères avec Réplication Massive (Kératite Stromale Nécrosante OU Endothélite Linéaire)
*   **Décision 1** : Hospitalisation en urgence.
*   **Décision 2** : Aciclovir Intraveineux (5 à 10 mg/kg toutes les 8h).
*   **Décision 3 (Corticoïdes)** :
    *   *Stromale Nécrosante* : 🛑 **AUCUN CORTICOÏDE** en phase aiguë (Risque majeur de fonte cornéenne et perforation).
    *   *Endothélite Linéaire* : ✅ Introduction de stéroïdes topiques, mais strictement retardée après confirmation de l'efficacité antivirale.

#### Sous-Branche 2B : Formes Immunologiques / Mixtes (Kératite Stromale Non Nécrosante, Endothélite Disciforme ou Diffuse, Uvéite Antérieure)
C'est la situation clinique la plus fréquente nécessitant un maniement subtil. L'objectif est d'encadrer l'immunité tout en bloquant le virus.
*   **Étape 1 (Attaque Antivirale)** : Valaciclovir Per Os (Pleine dose : 1g à 3g/jour selon sévérité, soit 2 à 6 comprimés de 500 mg).
*   **Étape 2 (Fenêtre d'observation)** : Attendre le début d'amélioration clinique (48h à 72h) sous couverture antivirale seule.
*   **Étape 3 (Introduction des Stéroïdes)** : Si et seulement si la réplication est bloquée (pas d'aggravation). Dexaméthasone topique à dose d'attaque (6 à 8 gouttes/jour).
*   **Étape 4 (Décroissance)** : Réduction progressive par paliers de 5 à 10 jours minimum. Maintien obligatoire du Valaciclovir PO pendant toute la décroissance.

---

### 🟪 BRANCHE 3 : GESTION DES COMPLICATIONS ET SEVRAGE (L'Arbre Décisionnel des Récidives)
Pendant la phase de décroissance des corticoïdes (Branche 2B) ou après guérison, voici l'algorithme décisionnel :

*   **Récidive inflammatoire AVANT la fin du sevrage en corticoïdes (Corticodépendance)** :
    *   *Action* : Arrêter la décroissance, remonter le corticoïde au palier précédent et introduire Ciclosporine A 2% collyre (hors AMM) pour prendre le relais immunomodulateur.
*   **Disparition de l'inflammation et sevrage réussi** :
    *   *Action* : Arrêt des stéroïdes.
*   **Survenue d'une 1ère récidive rapide après sevrage** :
    *   *Action* : Reprise immédiate du Valaciclovir PO (1 à 3 comprimés/jour).
*   **Survenue d'une 2ème récidive rapide OU ≥ 2 épisodes stromaux en moins de 1 an** :
    *   *Action* : Mise en route de la Prévention au long cours.
    *   *Protocole* : Valaciclovir 500 mg (1 comp/jour) pendant 12 mois minimum. Surveillance créatinine, bilan hépatique et NFS.

---

## 🎨 Représentation Synthétique (Code Couleurs d'Urgence)

<div className="table-wrapper">
| Situation Clinique | 🦠 Statut Viral | 💊 Action Antivirale Immédiate | 💧 Statut Corticoïdes |
| :--- | :--- | :--- | :--- |
| **Ulcère Fluo +** | Réplication Active | Topique ou PO (Dose curative) | 🛑 **INTERDIT** |
| **Nécrose Blanche** | Réplication Massive | Intraveineux (Hospitalisation) | 🛑 **INTERDIT** (Phase aiguë) |
| **Infiltrat Diffus sans Ulcère** | Réaction Immune | Per Os (Dose curative) obligatoire | ⏳ **OUI** (Mais après 48h d'antiviral) |
| **Œdème + Hypertonie** | Mixte (Trabéculite) | Per Os (Dose curative) | ⏳ **OUI** (Dès blocage viral confirmé) |
</div>
`;

export const hsvAlgoMermaid = `
graph TD
    classDef redFill fill:#ef4444,stroke:#fff,stroke-width:2px,color:#fff;
    classDef greenFill fill:#10b981,stroke:#fff,stroke-width:2px,color:#fff;
    classDef blueFill fill:#3b82f6,stroke:#fff,stroke-width:2px,color:#fff;
    classDef purpleFill fill:#8b5cf6,stroke:#fff,stroke-width:2px,color:#fff;
    classDef orangeFill fill:#f59e0b,stroke:#fff,stroke-width:2px,color:#fff;
    classDef grayFill fill:#374151,stroke:#fff,stroke-width:2px,color:#fff;

    Start((LAF : Prise <br>de Fluorescéine ?)):::grayFill -->|OUI| Epith(Atteinte Épithéliale<br>Dendrite / Géographique):::greenFill
    Start -->|NON| Deep(Atteinte Profonde<br>Stromale / Endothéliale / Uvéite):::blueFill

    Epith -->|Décision| CI_Cort[🛑 CONTRE-INDICATION<br>AUX CORTICOÏDES]:::redFill
    CI_Cort --> TTT_Epith(Antiviral Local OU Per Os<br>Contrôle J5-J7)

    Deep -->|K. Stromale Nécrosante<br>Endothélite Linéaire| Urgence(Forme Sévère :<br>Hospitalisation + Aciclovir IV):::orangeFill
    Urgence --> UrgenceCort[🛑 Pas de corticoïdes en phase aiguë]:::redFill

    Deep -->|K. Stromale Non Nécrosante<br>Endothélite Diffuse<br>Uvéite| Immune(Forme Immunologique / Mixte):::blueFill
    Immune --> Etape1[Étape 1 : Valaciclovir PO Pleine Dose]
    Etape1 --> Etape2[Étape 2 : Attendre 48h-72h]
    Etape2 --> Etape3{Amélioration ?<br>Réplication Bloquée ?}
    Etape3 -->|OUI| Cort(Introduction Dexaméthasone<br>Décroissance très lente):::purpleFill
    Etape3 -->|NON| Wait(Poursuivre Antiviral seul)

    Cort --> Recidive(Récidive inflammatoire<br>pendant sevrage ?)
    Recidive -->|OUI| Ciclo(Remonter corticoïde +<br>Ciclosporine A 2% relais)
    Recidive -->|NON, Guérison| StopCort(Arrêt des Stéroïdes)

    StopCort --> RecidiveApres(Récidive rapide ?)
    RecidiveApres -->|1ère fois| Reprise[Reprise immédiate<br>Valaciclovir PO]
    RecidiveApres -->|2ème fois / ≥2 par an| Prophylaxie[Prévention au long cours<br>Valaciclovir 500mg/j x 12 mois]:::purpleFill
`;

export const hsvClassificationsMarkdown = `
# 📊 Classifications : Herpès Oculaire

> [!NOTE]
> Bientôt disponible : Classification détaillée.
\`;

export const hsvClassificationsMermaid = \`
mindmap
  root((Herpès Oculaire))
    Épithélial
    Stromal
    Endothélial
\`;
